...
const getToken = async (code: string) => {
  ...
  if (!clientId) {
    ...
    console.error(
      'Note: If you just created/updated .env.local, you need to restart the Vite dev server for changes to take effect.'
    );
    return;
  }
  ...
  if (!scopesValid) {
    console.error(
      'Token does not have required scopes. Clearing tokens and requiring re-authentication.'
    );
