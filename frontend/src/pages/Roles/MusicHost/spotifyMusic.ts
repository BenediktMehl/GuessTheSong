import { getPlayer, getDeviceId, isReady } from './spotifyPlayer';
...
export type SpotifyResponseStatus =
  (typeof SpotifyResponseStatus)[keyof typeof SpotifyResponseStatus];
...
export async function playSpotifyTrack(trackUri: string) {
  ...
  try {
    ...
    console.warn('Error activating Spotify player element:', activationError);
  }
  ...
  try {
    ...
    console.error('Error playing track via Web API:', error, {
      errorType: error instanceof Error ? error.constructor.name : typeof error,
      errorMessage: error instanceof Error ? error.message : String(error),
      trackUri,
      deviceId,
...
export async function getPlaylistTracks(
  ...
  limit = 100,
  offset = 0
): Promise<SpotifyPlaylistTracksResponse | null> {
  const accessToken = localStorage.getItem('access_token');
  if (!accessToken) {
    console.error('No access token found in localStorage');
    return null;
