...
import {
  initializePlayer,
  subscribeToAuthenticationErrors,
...
type PlaylistTrack = SpotifyTrack & {
  track: NonNullable<SpotifyTrack['track']>;
};
...
export default function HostGame() {
      ...
      if (initialized) {
        ...
        unsubscribeAuthErrors = subscribeToAuthenticationErrors((message) => {
          ...
          if (message.includes('Invalid token scopes') || message.includes('scope')) {
            setErrorMessage(
              'Spotify authentication failed: Invalid token scopes. Please log out and log in again to grant the required permissions.'
            );
          } else {
        ...
        if (initialState?.item) {
          setCurrentTrack(initialState.item as CurrentTrack);
          setSpotifyStatus(
  ...
  useEffect(() => {
    const loadPlaylistTracks = async () => {
      ...
      if (response) {
        const sanitizedTracks = response.items.filter(
          (item): item is PlaylistTrack => item.track !== null
        );
        setPlaylistTracks(sanitizedTracks);
      }
  ...
  <div className="w-full max-w-md flex flex-col gap-6">
        ...
        <Card title="Settings" className="w-full" bodyClassName="flex flex-col gap-2">
          <label htmlFor="hide-song-toggle" className="label cursor-pointer">
              ...
              >
                ...
                {searchQuery.trim() && searchResults.length > 0 && (
                  <optgroup label="Search Results">
                    {searchResults
                      .filter(
                        (p) => p.id !== DEFAULT_PLAYLIST_ID && !playlists.some((up) => up.id === p.id)
                      )
                      .map((playlist) => (
                        <option key={playlist.id} value={playlist.id}>
                          {playlist.name}{' '}
                ...
                <div className="mt-2">
                  {loadingTracks ? (
                    ...
                    <div className="max-h-[200px] overflow-y-auto space-y-1">
                      {playlistTracks.slice(0, 20).map((item) => (
                        <button
                          key={item.track.id}
                          type="button"
      ...
      {showToast && spotifyStatus === SpotifyResponseStatus.PLAYING && !skippedTrack && !errorMessage && (
        <div className="toast toast-top toast-center">
      ...
      {showToast && spotifyStatus === SpotifyResponseStatus.PAUSED && !skippedTrack && !errorMessage && (
        <div className="toast toast-top toast-center">
